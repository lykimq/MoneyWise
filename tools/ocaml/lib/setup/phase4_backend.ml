...
These steps handle the "stateful" parts of the environment, such as runnning services,
verifying the database state. They can be implemented after the core logic is working.
...
- Step 5: Database Verification (Depend on step 3 and 4)
  Perfom checks to ensure the database is correctly set up and the connection is working.
  + Test connection:
    ++ For `Supabase`, verify the connection to the remote database.
    ++ For `Local`, verify the connection to the local PostgreSQL instance.
  + Verify Schema: Implement a basic schema check (e.g., query for the existence of a key
  table created by the migrations.)
  Depend on step 3, and for a local setup, the service must be running (step 4)
...
Step 6: Live Server Health Check (Depend on step 3)
  Briefly start the backend server and test its main API endpoints to confirm the build
  was successful and the server can run.
  + Start Server: Run `cargo run`
  + Health Check: After a short delay, use an OCaml HTTP client library to make requests
  to `http://localhost:3000/api/bugets/overview` and other key endpoints.
