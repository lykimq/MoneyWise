...
let print_header text =
  ...
  Logs.app (fun m -> m "%s" (String.make (String.length text) '='))
...
let run_verification (root_dir : string) : Types.setup_result =
  ...
  try
    ...
    Logs.app (fun m -> m "Project directory: %s" root_dir) ;
    ...
    Logs.app (fun m -> m "✅ Initial verification passed successfully!") ;
    ...
    let print_list_section_if_any title items =
      if items <> [] then (
        print_section title ;
        List.iter (fun s -> Logs.app (fun m -> m "• %s" s)) items )
    in
    let print_next_steps (r : Types.setup_result) =
      if r.success then (
        print_section "🚀 What's Ready" ;
        Logs.app (fun m -> m "• Project structure: ✅ Verified") ;
        Logs.app (fun m -> m "• Prerequisites: ✅ Installed") ;
        Logs.app (fun m -> m "• Development environment: Ready for setup") ;
        ...
        Logs.app (fun m -> m "3. Environment Config:      moneywise env-setup") ;
        Logs.app (fun m ->
            m "4. Service Management:      moneywise services-setup" ) ;
        Logs.app (fun m -> m "5. Final Validation:        moneywise verify") ;
        Logs.app (fun m ->
            m "💡 Tip: Use 'moneywise --help' to explore all available commands" )
        )
      else (
        print_section "🔄 Next Steps" ;
        Logs.app (fun m -> m "1. Address the errors listed above") ;
        Logs.app (fun m -> m "2. Run 'moneywise verify' to verify fixes") ;
        Logs.app (fun m -> m "3. Run setup commands as needed") )
    in
