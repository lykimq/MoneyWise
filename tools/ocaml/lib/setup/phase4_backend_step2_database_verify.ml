...
let check backend_dir =
  ...
  if not (Utils.file_exists env_path) then (
    Logs.err (fun m -> m ".env file not found at %s" env_path);
    result :=
      Errors.add_phase_error !result
        (Fmt.str ".env file not found. Please run step 1 first.");
    Error !result)
  else (
    ...
    | Ok env_vars -> (
        ...
        | Ok database_url ->
            ...
            result :=
              Errors.add_detail !result
                (Fmt.str "Database type: %s" db_type_str);
            ...
            | Unknown ->
                Logs.warn (fun m ->
                    m "Unknown database type - assuming Supabase");
                result :=
