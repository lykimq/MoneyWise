(** Phase 1: Project Structure Verification This phase ensures that the
    MoneyWise project has all required directories and files before proceeding
    with further setup phases. This verification is critical because:

    1. Missing core directories (backend/frontend) would prevent proper
    application setup 2. Missing setup scripts would break the automated
    installation process 3. Missing configuration files would lead to runtime
    errors
...
let create_empty_status () =
  { total_checks = 0; passed_checks = 0; failed_checks = 0; results = [] }
...
let check_single_path root_dir path description exists_fn =
  ...
  let error_message =
    if not exists then
      Some
        (Fmt.str "%s not found at %s" description (Utils.format_path full_path))
    else None
  in
  { path = full_path; description; exists; error_message }
...
let display_structure_status status =
  ...
  List.iter
    (fun result ->
      ...
      let error_str =
        match result.error_message with Some msg -> " - " ^ msg | None -> ""
      in
      Utils.log_at_current_level
        (Printf.sprintf "%s %s%s" icon result.description error_str))
    (List.rev status.results);
  Utils.log_at_current_level "";
  Utils.log_at_current_level
    (Printf.sprintf "Summary: %d/%d checks passed" status.passed_checks
       status.total_checks);
  if status.failed_checks = 0 then
    Utils.log_at_current_level "✅ All structure checks passed successfully!"
  else
    Utils.log_at_current_level
      "❌ Some required paths are missing. Please check the errors above."
