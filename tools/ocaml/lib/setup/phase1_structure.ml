...
let check_single_path root_dir path description exists_fn =
  ...
  let error_message =
    if not exists then
      Some
        (Fmt.str "%s not found at %s" description
           (Utils.format_path full_path) )
    else None
  in
...
let display_structure_status status =
  ...
  List.iter
    (fun result ->
      ...
      in
      Logs.app (fun m -> m "%s %s%s" icon result.description error_str) )
  ...
  if status.failed_checks = 0 then
    Logs.app (fun m -> m "✅ All structure checks passed successfully!")
  else
    Logs.app (fun m ->
        m "❌ Some required paths are missing. Please check the errors above." )
