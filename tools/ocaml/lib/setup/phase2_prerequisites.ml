...
let display_prerequisites_status status =
  Logs.app (fun m -> m "🔍 Checking MoneyWise project prerequisites...") ;
  Logs.app (fun m -> m "============================================") ;
  List.iter
    (fun result ->
      ...
      let error_str =
        ...
        | None ->
            ""
      in
      Logs.app (fun m -> m "%s %s%s%s" icon result.name version_str error_str) )
    status.results ;
  Logs.app (fun m -> m "") ;
  Logs.app (fun m ->
      m "Summary: %d/%d checks passed" status.passed_checks status.total_checks ) ;
  if status.failed_checks = 0 then
    Logs.app (fun m -> m "✅ All prerequisites verified successfully!")
  else
    Logs.app (fun m ->
