...
let display_prerequisites_status status =
  Utils.log_at_current_level "🔍 Checking MoneyWise project prerequisites...";
  ...
  List.iter
    (fun result ->
      ...
      let version_str =
        ...
        | Some v -> Fmt.str " (version: %s)" v
        | None -> ""
      in
      let error_str =
        match result.error_message with
        | Some msg -> Fmt.str " - %s" msg
        | None -> ""
      in
      Utils.log_at_current_level
        (Printf.sprintf "%s %s%s%s" icon result.name version_str error_str))
    status.results;
  Utils.log_at_current_level "";
  Utils.log_at_current_level
  ...
  if status.failed_checks = 0 then
    Utils.log_at_current_level "✅ All prerequisites verified successfully!"
  else
    Utils.log_at_current_level
