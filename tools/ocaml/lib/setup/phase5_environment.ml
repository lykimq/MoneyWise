...
let configure_environment root_dir =
  ...
  let check_env_script script_name description =
    ...
    if file_exists script_path then (
      ...
      result :=
        add_phase_warning !result (Printf.sprintf "%s not found" description);
      false)
  in
      ...
      if exists then (
        ...
        if make_executable script_path then
          ...
          if exit_code = 0 then (
            ...
            result :=
              add_detail !result
                (Printf.sprintf "%s completed successfully" description))
          else (
            Printf.printf "    ⚠️  %s had issues (exit code: %d)\n" description
              exit_code;
            result :=
              add_phase_warning !result
                (Printf.sprintf "%s had issues (exit code: %d)" description
                   exit_code))
        else (
          Printf.printf "    ⚠️  Failed to make %s executable\n" description;
          result :=
            add_phase_warning !result
              (Printf.sprintf "Failed to make %s executable" description))))
    script_status;
